...
int main(int argc, char** argv)
{
    ...
    while (!g.getQuit()) {
        ...
        for (int i = 0; i < NUMBER_OF_LOGS; i++) {
            ...
            else if(logLeft[i].inOn(frog.getLR())){
                frog.move(logLeft[i].getSpeed());
                onLog = true;
            }
            else if(logRight[i].inOn(frog.getLR())){
                frog.move(logRight[i].getSpeed());
                onLog = true;
            }
        }
        ...
        //off the screen?
        if (frog.getLocation().getX() >= DISPLAY_WIDTH - frog.getSize()) {
            isDead = true;
        }
        else if (frog.getLocation().getX() < 0) {
            isDead = true;
        }
        else if (frog.getLocation().getY() < 0) {
            isDead = true;
        }
        else if (frog.getLocation().getY() > DISPLAY_HEIGHT) {
            isDead = true;
        }
